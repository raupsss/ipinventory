...
function myheader($kiri,$kanan,$judul,$tgl_1,$tgl_2){
?>
<div class="atas">
<table width="100%">
<tr>
...
function myfooter(){	
	...
	$beli = $this->app_model->CariJmlBeli($r['kode_barang'],$tgl_awal,$tgl_akhir);
	$jual = $this->app_model->CariJmlJual($r['kode_barang'],$tgl_awal,$tgl_akhir);
	$stok = ($awal+$beli)-$jual;// $this->app_model->CariStokAkhir($db['kode_barang']);
	$gudang = $this->app_model->Nama_Gudang($r['id_gudang']);
	if(($no%25) == 1){
   	if($no > 1){
        myfooter();
   	...
   	myheader($kiri,$kanan,$judul,$tgl_1,$tgl_2);
	}
	?>
    <tr>
