...
class Pengguna extends CI_Controller {
	...
	public function tambah()
	{
		...
		if(!empty($cek)){
			...
			$d['l_gudang'] = $this->app_model->manualQuery("SELECT * FROM gudang");
	...
	public function simpan()
	{
		...
		if(!empty($cek)){
				...
				if($data->num_rows()>0){
					if(empty($pwd)){
						$this->app_model->manualQuery("UPDATE admins SET nama_lengkap='$nama',level='$level',id_gudang='$gudang' WHERE username='$user'");
					}else{
