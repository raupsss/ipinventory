...
class absensi_pgt1 extends CI_Controller {
		...
		if(!empty($cek)){
			...
			if(empty($cari) && empty($gudang)){
				...
				if(!empty($cari)){
					...
					$where = " WHERE kode_barang LIKE '%$cari%' OR nim LIKE '%$cari%'";
				}else{
			...
			$text = "SELECT * FROM absensi_pgt1 $where ";		
			$tot_hal = $this->app_model->manualQuery($text);		
			
			$d['tot_hal'] = $tot_hal->num_rows();
			
			$config['base_url'] = site_url() . '/absensi_pgt1/index/';
			...
			$d['content'] = $this->load->view('absensi_pgt1/view', $d, true);		
